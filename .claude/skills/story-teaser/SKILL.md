...
썸네일 4장이 다 글씨투성이여도 대개 살릴 수 있다 (실측 2026-08):
- 글씨가 **위·아래 띠**에 몰려 있으면 → 가운데를 16:9로 크롭 (가장 흔한 경우)
- 글씨가 **화면 전체를 가로지르면** → 글씨 없는 사분면만 작게 크롭. 3.5배까지 확대해도
  **선화(펜 드로잉)는 Lanczos + 언샵 75%로 충분히 버틴다.** 사진이면 포기할 것.
- 크롭 후 반드시 `Read`로 확인 — 띠 가장자리에 글자 끝이나 색 띠가 남기 쉽다.
  남았으면 그 방향으로 15~20px 더 안쪽에서 다시 자른다.

crop → Lanczos 확대 → 언샵으로 복원한다 (띠 위치가 애매하면 행별 밝은 픽셀 수로 찾는다):
...
판단은 눈이 아니라 숫자로 한다 — 크롭 후 **자막대(820~1000행) 평균 밝기가 80을 넘으면**
흰 자막이 묻힌다. 80 아래로 떨어질 때까지 강도를 올린다:
`np.asarray(Image.open(f).convert("L"))[820:1000].mean()`
...
**나레이션이 있으면 `master_audio.py`로 마감한다 (2026-09부터 기본):**

```bash
python3 $S/master_audio.py renders/<렌더>.mp4 delays.json voice bgm.wav 최종.mp4
```

한 번에 하는 일:
- **더킹**: 나레이션이 나오는 동안만 BGM을 일정하게 -9dB 내리고 말 사이엔 되돌린다.
  나레이션 위치(delays.json)를 이미 알기 때문에 시각 기반 곡선으로 한다 —
  ffmpeg 자동 감지(sidechaincompress)는 실측에서 문장마다 -7~-20dB로 들쭉날쭉했다.
- **목소리 다듬기**: 저역 잡음 제거, 발음 대역 +2.5dB, 컴프레서, 짧은 예배당 잔향
  (`--reverb=none`으로 끌 수 있음 — 설교 톤이 아닌 사연물이면 끄는 게 낫다)
- **유튜브 표준 음량**: 2패스 loudnorm으로 **-14 LUFS / 최대 -2.0 dBTP** 이하.
  예전 고정 볼륨 방식은 실측 **-19.6 LUFS** — 유튜브에서 다른 영상보다 체감 절반쯤
  작게 들렸다. 스크립트가 끝에 실측 음량을 출력하니 목표와 맞는지 확인할 것.

나레이션 없이 BGM만이면 아래 명령으로 충분하다:
...
`cinematic` 기본 강조색은 붉은색이다 — 공포·미스터리엔 맞지만, **눈물·감동·빛**의 이야기
(권사님, 새벽기도, 가족)에서는 섬뜩하게 읽힌다. 그럴 땐 `"em_color": "#f4c56a"`(따뜻한 금색).
강조어 뒤 여백은 `cap_backdrop`을 쓸 때만 생긴다 — 예전엔 항상 붙어서
"마지막 한마디 를"처럼 조사가 띄어져 보였다 (2026-09 수정).

그림 속 **이름표·말풍선 같은 덧붙인 글씨**는 인페인팅으로 지운다(썸네일 큰 글씨와 같은 방법).
특히 그림 속 이름이 대본과 다르면(예: 그림 "정선임" / 대본 "정순임") 반드시 지우고,
자막에서도 이름 대신 "권사님"처럼 호칭을 쓴다. 인페인팅이 무늬 있는 옷 위에서
검은 얼룩으로 뭉개지면, 바로 옆 같은 옷감을 떠서 가장자리를 흐리게 덮는다(복제 도장).

## 화면 움직임 (켄번즈 줌 + 팬)

기본은 3~4초에 걸쳐 1.02 → 1.16배로 천천히 미는 은은한 줌이다. 카스가
*"원래 움직임은 없었나?"* 라고 물은 적이 있다 — 차분한 사연물엔 맞지만
임팩트가 필요하면 spec에서 올린다 (샷별/전체 둘 다 지정 가능):

| 필드 | 뜻 | 권장 |
|---|---|---|
| `zoom_amt` | 줌 폭 (기본 0.14) | 0.20~0.22면 확실히 밀려든다 |
| `zoom_base` | 시작 배율 (기본 1.02) | **팬을 쓰려면 1.08 이상** |
| `pan` | `"left"` / `"right"` | 확대 여백만큼 가로로 훑는다 |

**팬은 `zoom_base`가 1보다 충분히 커야 움직인다.** 이동 폭은 화면 밖으로 밀려난
여백에서 나오는데, 1.02면 여백이 19px뿐이라 눈에 안 띈다. 1.08이면 60px대가 되어
3초 동안 천천히 흐르는 것이 보인다. 생성기가 `focus`(확대 기준점)까지 계산해
**좁은 쪽 여백의 90%까지만** 쓰므로 검은 띠는 생기지 않는다.

검증: 팬이 최대로 치우치는 순간(샷 시작·끝 0.05초 안쪽)을 스냅샷으로 찍어
좌우 최외곽 3픽셀 평균 밝기가 2 미만이면 띠가 생긴 것이다.
